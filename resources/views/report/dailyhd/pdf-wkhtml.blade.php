<html>
	<head>
		...
		<title>.:: Rekapitulasi Operasional Alat Berat ::.</title>
		<style>
			body{
				font-size: 13px;
			}
			...
			<table class="table-product" style="width: 100%;"  >
				<thead>
					<tr>
						...
						<th style="border:0.5px solid #000;text-align: center;" >TANGGAL</th>
						@if($group_by != 'alat_id')
							<th style="border:0.5px solid #000;text-align: center;" >ALAT</th>
						@endif
						@if($group_by != 'lokasi_galian_id')
							<th style="border:0.5px solid #000;text-align: center;" >LOKASI GALIAN</th>
						@endif
						@if($group_by != 'pengawas_id')
							<th style="border:0.5px solid #000;text-align: center;" >PENGAWAS</th>
						@endif
						@if($group_by != 'operator_id')
							<th style="border:0.5px solid #000;text-align: center;" >OPERATOR</th>
						@endif
						<th style="border:0.5px solid #000;text-align: center;" >SOLAR</th>
						<th style="border:0.5px solid #000;text-align: center;" >OLI</th>
						<th style="border:0.5px solid #000;text-align: center;" >TOTAL<br/>JAM KERJA</th>
					</tr>
				</thead>
				<tbody>
					<?php $grand_solar = 0; ?>
					...
					@foreach($data_group as $pg)
						...
						<tr>
							@if($tipe_report == 'detail')
								<td colspan="8" style="border:0.5px solid #000;background-color: azure;" >
									<strong>
						...
						@if(isset($pg->detail))
							...
							<tr>
								...
								@if($group_by != 'alat_id')
									<td style="border:0.5px solid #000;">{{$dt->kode_alat . ' - ' . $dt->alat}}</td>
								@endif
								@if($group_by != 'lokasi_galian_id')
									<td style="border:0.5px solid #000;">{{$dt->lokasi_galian}}</td>
								@endif
								@if($group_by != 'pengawas_id')
									<td style="border:0.5px solid #000;">{{$dt->kode_pengawas . ' - ' . $dt->pengawas}}</td>
								@endif
								@if($group_by != 'operator_id')
									<td style="border:0.5px solid #000;">{{$dt->kode_operator . ' - ' . $dt->operator}}</td>
								@endif
								<td style="text-align: right;border:0.5px solid #000;" >
									{{$dt->solar}}
								</td>
							...
							@endforeach
							<tr>
								<td style="text-align: right;border:0.5px solid #000;" colspan="5" ><strong>TOTAL {{$subtitle_val}}</strong>
								</td>
								<td style="text-align: right;border:0.5px solid #000;" >
					...
					<tr>
						<td colspan="5" style="text-align: center;border:0.5px solid #000;height: 30px;vertical-align: middle;" >
							<strong>TOTAL</strong>
						</td>
