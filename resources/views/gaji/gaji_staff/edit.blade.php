...
<section class="content-header">
    <h1>
        <a href="gaji/gaji-staff" >Gaji Staff</a> <i class="fa fa-angle-double-right" ></i> {{$data->state == 'draft' ? 'New' : $data->name}}
    </h1>
</section>
...
<section class="content">
    <div class="box box-solid">
        <form role="form" method="POST" action="gaji/gaji-staff/update" >
            <input type="hidden" name="gaji_staff_id" value="{{$data->id}}">
            ...
            <div class="box-body">
                <div class="box-body">
                    ...
                    <div class="alert alert-warning alert-dismissible">
                        <h5><i class="icon fa fa-warning"></i> Anda memiliki tagihan piutang kepada Staff ini sebesar Rp. <span class="uang" >{{$piutang}}</span> <a class="pull-right" href="" >check Out!</a></h5>
                    </div>
                    @endif
                    <div class="row" >
                        <div class="col-xs-6" >
                            ...
                            <table class="table table-condensed no-border" >
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="col-xs-6"  ><input name="jumlah" class="text-right text-bold form-control no-border no-padding " style="background-color: white;" readonly id="total-jumlah" value="{{($data->masuk_pagi+$data->masuk_siang)/2}}" ></td>
                                    </tr>
                                    <tr style="border-top: thin solid darkgrey;" class="hide" >
            ...
            <div class="box-footer" >
                ...
                @if($data->state != 'draft')
                    <a class="btn btn-warning" id="btn-cancel" data-href="gaji/gaji-staff/to-cancel/{{$data->id}}" href="{{url()->current()}}#btn-cancel" ><i class="fa fa-reply-all" ></i> Cancel</a>
                @endif

                <a class="btn btn-danger" href="gaji/gaji-staff" ><i class="fa fa-close" ></i> Close</a>

                @if($data->state == 'draft')
                    <a href="gaji/gaji-staff/confirm/{{$data->id}}" class="btn bg-maroon pull-right" id="btn-confirm"  ><i class="fa fa-check" ></i> Confirm</a>
                @endif

                @if($data->state == 'open' && $data->dp == 'N')
                    <a class="btn bg-purple pull-right " id="btn-validate" href="gaji/gaji-staff/validate/{{$data->id}}" ><i class="fa fa-check" ></i> Validate</a>
                @endif
                ...
                @if($data->state == 'draft')
                    <a class="text-red btn pull-right " id="btn-delete" style="margin-right: 10px;" data-href="gaji/gaji-staff/delete/{{$data->id}}" href="{{url()->current()}}#btn-delete" ><i class="fa fa-close" ></i> Delete</a>
                @endif

                @if($data->state == 'paid' && $data->dp == 'N')
                <a class="btn bg-green pull-right " id="btn-print-pdf" href="gaji/gaji-staff/print-pdf/{{$data->id}}" target="_blank" ><i class="fa fa-file-pdf-o" ></i> PDF</a>
                @endif
            </div>
        </form>
    </div><!-- /.box -->
...
(function ($) {
    ...
    $('input[name=bulan]').datepicker({
        ...
        // var form = $('<form>').attr('method','POST').attr('action','gaji/gaji-staff/get-pay-day').append($('input').attr('name','bulan').attr('type','text').val($('input[name=bulan]').val()));
        // $('body').append(form);
        // form.submit();
    ...
    $('form').submit(function(){
        ...
        table.find('tbody').find('tr').each(function(i,data){
            item = {};
            item['gaji_staff_detail_id'] = $(this).data('detailid');
    ...
    $('#btn-save-dp').click(function(){
        jsonData = {};
        jsonData['gaji_staff_id'] = $('input[name=gaji_staff_id]').val();
        jsonData['jumlah'] = $('input[name=dibayar]').autoNumeric('get');

        // $.post('gaji/gaji-staff/save-payment',{
        ...
        // using form
        form = $('<form>').attr('method','POST').attr('action','gaji/gaji-staff/save-payment');
    ...
    function getPaymentInfo(){
        var res='';
        $.ajax({
            url:'gaji/gaji-staff/get-payment-info/' + $(this).data('paymentid'),
            method:"GET",
            async:false,
