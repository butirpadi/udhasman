...
class SalesOrderController extends Controller
{
	...
	public function cetak($id){
		$data = \DB::table('view_sales_order')->find($id);
		$data->detail = \DB::table('view_sales_order_detail')->whereSalesOrderId($id)->get();
        ...
        $space_len = strlen($data->order_number) - strlen($data->customer);
        $spaces_1 = $space_len > 0 ? 0 : abs($space_len);
        $spaces_2 = $space_len > 0 ? abs($space_len) : 0;

        $Data .= str_repeat(' ',$spaces_1+10) . "Tanggal       :  ";
        $Data .= $data->order_date_formatted ."\n" ;

        $Data .= "Customer      :  ";
        $Data .= $data->customer ;

        $Data .= str_repeat(' ',$spaces_2+10) . "Pekerjaan     :  ";
        $Data .= $data->pekerjaan ."\n" ;

        $Data .= "Alamat        :  ";
        $Data .= $data->alamat_pekerjaan.', ' .$data->desa .', '.$data->kecamatan .', '.$data->kabupaten .', '.$data->provinsi ."\n" ;
	...
	public function pdf($id){
		...
		// generate header
		GeneratePdfHeader($pdf,'Sales Order',$data->order_number);

		// genereate content header
		$pdf->SetFont('Arial','B',8);
		$pdf->SetTextColor(0,0,0);
		$pdf->SetXY(8,$pdf->GetY()+5);
		$pdf->Cell(20,4,'Customer',0,0,'L',false);
		$pdf->Cell(2,4,':',0,0,'L',false);
		$pdf->SetFont('Arial',null,8);
		$pdf->Cell($pdf->GetStringWidth($data->customer),4,$data->customer,0,0,'L',false);

		$pdf->SetX(100);
		$pdf->SetFont('Arial','B',8);
		$pdf->Cell(20,4,'Tanggal Order',0,0,'L',false);
		$pdf->Cell(2,4,':',0,0,'L',false);
		$pdf->SetFont('Arial',null,8);
		$pdf->Cell($pdf->GetStringWidth($data->order_date_formatted),4,$data->order_date_formatted,0,2,'L',false);

		$pdf->SetX(8);
		$pdf->SetFont('Arial','B',8);
		$pdf->Cell(20,4,'Pekerjaan',0,0,'L',false);
		$pdf->Cell(2,4,':',0,0,'L',false);
		$pdf->SetFont('Arial',null,8);
		$pdf->Cell($pdf->GetStringWidth($data->pekerjaan),4,$data->pekerjaan,0,0,'L',false);

		$pdf->SetX(100);
		$pdf->SetFont('Arial','B',8);
		$pdf->Cell(20,4,'Alamat',0,0,'L',false);
		$pdf->Cell(2,4,':',0,0,'L',false);
		$pdf->SetFont('Arial',null,8);
		$data->alamat_pekerjaan = "Jl. Ngaban RT 5 Rw 2 No. 15";
		$data->desa = "Ngaban";
		$data->kecamatan = "Tanggulangin";
		$data->kabupaten = "Sidoarjo";
		$data->provinsi = "Jawa Timur";
		$alamat = $data->alamat_pekerjaan.', ' . $data->desa . ', '.$data->kecamatan.', '.$data->kabupaten.','.$data->provinsi;
		$pdf->MultiCell(85,4,$alamat,0,'L',false);

		// TABLE HEADER
		$content_width = $pdf->GetPageWidth()-16;
		$separator_width = 1;
		$table_width = $content_width - (2*$separator_width); 
		...
		$pdf->Cell($col_no,$header_height,'NO',0,0,'C',true);
		$pdf->Cell($separator_width,$header_height,null,0,0,'C',false);

		$pdf->SetFillColor(0,128,128);
		$pdf->Cell($col_material,$header_height,'MATERIAL',0,0,'C',true);
		$pdf->Cell($separator_width,$header_height,null,0,0,'C',false);
		
		$pdf->SetFillColor(0,0,0);
		$pdf->Cell($col_qty,$header_height,'QUANTITY',0,0,'C',true);
		$pdf->Cell($separator_width,$header_height,null,0,2,'C',false);

		// LINE HEADER
		$pdf->SetX(8+$col_no+$separator_width);
		$pdf->SetFillColor(0,128,128);
		$pdf->Cell($col_material,1,null,0,2,'C',true);
		$pdf->SetX(8);
		$pdf->Cell($content_width,1,null,0,2,'C',true);
		...
		foreach($data->detail as $dt){
			...
			$pdf->Cell($col_no, $table_content_height,$rownum++,0,0,'C',false);
			$pdf->Cell($separator_width,$table_content_height,null,0,0,'C',false);

			$pdf->Cell($col_material, $table_content_height,$dt->material,0,0,'L',false);
			$pdf->Cell($separator_width,$table_content_height,null,0,0,'C',false);

			$pdf->Cell($col_qty, $table_content_height,$dt->qty,0,0,'R',false);
			$pdf->Cell($separator_width,$table_content_height,null,0,2,'C',false);

		}

		// LAST ROW LINE
		$pdf->SetX(8);
		$pdf->Cell($content_width,0.5,null,0,2,'C',true);

		$pdf->Output('I','SalesOrder.pdf');
		exit;
	}

	public function pdfCopy($id){
		$data = \DB::table('view_sales_order')->find($id);
		$data->detail = \DB::table('view_sales_order_detail')->whereSalesOrderId($id)->get();
		...
		$this->pdfCopyContent($pdf,$data);

		// get copy number
		$copy_number = Appsetting('slip_copy_number');

		for($i=0;$i<$copy_number;$i++){
			$pdf->AddPage();
			// generate header copy
			GeneratePdfHeader($pdf,'Sales Order [Copy ' . ($i+1) . ']',$data->order_number);
			$this->pdfCopyContent($pdf,$data);
		}
	...
	private function pdfCopyContent(&$pdf,$data){
		// genereate content header
		$pdf->SetFont('Arial','B',8);
		$pdf->SetTextColor(0,0,0);
		$pdf->SetXY(8,$pdf->GetY()+5);
		...
		$pdf->Cell(2,4,':',0,0,'L',false);
		$pdf->SetFont('Arial',null,8);
		$pdf->Cell($pdf->GetStringWidth($data->pekerjaan),4,$data->pekerjaan,0,2,'L',false);
